...
def DownloadImage(key_url, out_dir = 'assets/train/'):
  (key, url) = key_url
...
def Run_train(start=96):
    existing = os.listdir('assets/train/')
    existing_index = {key:index for index, key in enumerate(existing)}
    key_url_list = ParseData('assets/train.json')

    for key_url in tqdm(key_url_list[start:]):
        if key_url[0]+'.jpg' not in existing_index:
            DownloadImage(key_url,'assets/train/')
            existing_index[key_url[0]+'.jpg']=len(existing_index)
