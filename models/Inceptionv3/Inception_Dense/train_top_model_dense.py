...
fns = os.listdir('assets/bn_inception/')
random.shuffle(fns)
split_at = len(fns)//10
...
def data_gen_train():
    ...
    while True:
        for fn in fns_train:
            with open('assets/bn_inception/' + fn,'rb') as f:
...
inp = Input(shape=(5,5,2048))
avg = GlobalAveragePooling2D()(inp)
max_ = GlobalMaxPool2D()(inp)
main = layers.Concatenate()([avg,max_])
#main = Flatten()(main)
main = Dense(1024)(main)
...
check_point = ModelCheckpoint('models/Inceptionv3/Inception_Dense/top_model.hdf5', monitor="val_loss", mode="min", save_best_only=True, verbose=1)
