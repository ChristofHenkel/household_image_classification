...
def random_contrast(x, contrast_range):
...
def random_saturation(x, saturation_range):
    if len(saturation_range) != 2:
        raise ValueError('`saturation_range should be tuple or list of two floats. '
                         'Received arg: ', saturation_range)

    x = array_to_img(x)
    x = imgenhancer_Contrast = ImageEnhance.Color(x)
    u = np.random.uniform(saturation_range[0], saturation_range[1])
    x = imgenhancer_Contrast.enhance(u)
    x = img_to_array(x)
    return x
...
class CustomImageDataGenerator(object):
    ...
    def __init__(self,
        ...
        self.hard_center = hard_center
        self.hard_std_normalization = hard_std_normalization
    ...
    def standardize(self, x):
        ...
        if self.hard_center is not None:
            x -= self.hard_center

        if self.hard_std_normalization is not None:
            x /= (self.hard_std_normalization + K.epsilon())
    ...
    def random_transform(self, x, seed=None):
        ...
        if self.contrast_range is not None: #####
            x = random_contrast(x, self.contrast_range) #####

        if self.saturation_range is not None: #####
            x = random_saturation(x, self.saturation_range) #####
